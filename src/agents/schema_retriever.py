...
# from .sql_query_generator import sql_query_generator_node  # Import sql_query_generator_node
# from .data_operator import data_operator_node  # Add this import to fix the error
# import yaml
# import logging
...
#     # After schema is retrieved, generate SQL
#     if state.get("schema") and not state.get("generated_sql"):
#         return "sql_query_generator"
    
#     if state.get("generated_sql") and not state.get("query_results"):
...
#     from .data_operator import data_operator_node  # Add this import
...
#     graph.add_node("sql_query_generator", sql_query_generator_node)
#     graph.add_node("data_operator", data_operator_node)  # Add this node
...
#     graph.add_edge("sql_query_generator", "data_operator")  # Add this edge
...
#             "extracted_entities": [],
#             "generated_sql": ""  # Add this field
#         }
#         final_state = graph.invoke(initial_state)
#         logger.info(f"Agent completed: schema={final_state.get('schema', '')}")
#         return final_state  # Return the full state instead of just schema
#     except Exception as e:
...
from tools.db_utils import update_db_and_vector_store
from .state import AgentState
from agents.data_operator import data_operator_node
import yaml
import logging
...
def build_graph():
    ...
    graph.add_node("action", action_node)
    graph.add_node("sql_query_generator", sql_query_generator_node)
    graph.add_node("data_operator", data_operator_node)  # Assuming data_operator_node is imported or defined
...
def run_agent(nl_text):
    ...
    try:
        ...
        initial_state = {
            ...
            "extracted_entities": [],
            "generated_sql": "",  # Add this field
            "retry_count": 0,     # Initialize retry_count to prevent TypeError
            "current_max_tokens": 1000,  # Initialize max_tokens
            "query_results": None  # Initialize query_results
        }
        final_state = graph.invoke(initial_state)
        logger.info(f"Agent completed: schema={final_state.get('schema', '')}")
        return final_state  # Return the full state instead of just schema
