...
def verify_sql_with_deepseek(api_key, nl_text, generated_sql, schema, sql_command_type):
    ...
    logger.info(f"Calling Gemini flash API for SQL verification: {nl_text}")
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Gemini returned invalid JSON: {result_content}"
            )
        ...
        logger.exception(f"Gemini verification error: {e}")
